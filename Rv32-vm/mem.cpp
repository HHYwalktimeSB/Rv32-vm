...
{
	__try {
		*reinterpret_cast<unsigned short*>(membase + (_addr & _mask)) = bl_endian_switch16(val);
	}
	__except (GetExceptionCode() == EXCEPTION_ACCESS_VIOLATION ?
		EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH) {
		VirtualAlloc(membase + (_addr & _mask), 2, MEM_COMMIT, PAGE_READWRITE);
...
{
	__try {
		*reinterpret_cast<unsigned int*>(membase + (_addr & _mask)) = bl_endian_switch32(val);
	}
	__except (GetExceptionCode() == EXCEPTION_ACCESS_VIOLATION ?
		EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH) {
		VirtualAlloc(membase + (_addr & _mask), 4, MEM_COMMIT, PAGE_READWRITE);
